...
type DisplayMessage struct {
	// ID uniquely identifies the health issue so that clients can modify their
	// behaviour for particular issues (e.g. "session-expired").
	ID string
...
// Compare implements comparison for sorting for DisplayMessage. The ordering is
// arbitrary, but we need to have an ordering so we can compare two lists of
// DisplayMessages for equality.
func (m DisplayMessage) Compare(o DisplayMessage) int {
	return cmp.Or(
		cmp.Compare(m.ID, o.ID),
		cmp.Compare(m.Title, o.Title),
		cmp.Compare(m.Text, o.Text),
		cmp.Compare(m.Severity, o.Severity),
		bools.Compare(m.ImpactsConnectivity, o.ImpactsConnectivity),
	)
}
